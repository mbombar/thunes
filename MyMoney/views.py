...
def show_balance(request, gid):
    ...
    if users_balance is None or transactions is None:
        users = get_user_model().objects.filter(groups=group)
        ...
        for expense in expenses:
            ...
            value = Fraction(
                int(expense.value * expense.currency.value_in_eur * 100), 100
            )
...
def new_expense(request, gid):
    ...
    currency, _ = models.Currency.objects.get_or_create(
        name="Eur", defaults={"value_in_eur": 1, "symbol": "€"}
    )
    expense_form = ExpenseForm(
        request.POST or None,
        group=group,
        initial={"origin": request.user, "currency": currency},
    )
...
def edit_expense(request, gid, pk):
    ...
    ShareFormSet = modelformset_factory(
        models.Share, fields=("value", "owner"), extra=0
    )
    ...
    if expense_form.is_valid() and share_formset.is_valid():
        ...
        send_notification(
            expense, change=True, old_shares=old_shares, old_value=old_value
        )

        return redirect(reverse("MyMoney:index-expense", kwargs={"gid": gid}))
